    ...
    return (
      <div className="p-3 rounded border border-gray-300 bg-white">
        ...
        <div className="space-y-3">
          ...
          <LabeledInput label={wfRegistry.getWorkflowFieldLabel(WF_KEY, 'Name')} value={info.name} placeholder="例如：站厅" onChange={(v) => setInfo((p) => ({ ...p, name: v }))} />
          ...
          <LabeledInput label={wfRegistry.getWorkflowAuxLabel(WF_KEY, 'abbr')} value={info.abbr} placeholder="例如：HALL" onChange={(v) => setInfo((p) => ({ ...p, abbr: v }))} />
          <LabeledInput label={wfRegistry.getWorkflowFieldLabel(WF_KEY, 'nomenclator')} value={info.nomenclator} placeholder="例如：YZ" onChange={(v) => setInfo((p) => ({ ...p, nomenclator: v }))} />

          <div className="grid grid-cols-2 gap-2">
            <WorkflowFeatureSearchSelect
              bridge={bridge}
              label={wfRegistry.getWorkflowFieldLabel(WF_KEY, 'BuildingID')}
            ...
            />
            <LabeledInput label={wfRegistry.getWorkflowFieldLabel(WF_KEY, 'NofFloor')} value={info.nofFloor} placeholder="例如：B1 / 1 / 2" onChange={(v) => setInfo((p) => ({ ...p, nofFloor: v }))} />
          </div>
