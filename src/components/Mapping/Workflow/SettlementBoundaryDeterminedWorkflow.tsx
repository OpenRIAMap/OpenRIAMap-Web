// File: src/components/Mapping/Workflow/SettlementBoundaryDeterminedWorkflow.tsx
...
/**
 * SettlementBoundaryDeterminedWorkflow（工作流：聚落范围-确定范围）
 *
 * 基于 NaturalLandWorkflow 的结构：
 * 1) 填写者信息（CreateBy）
 * 2) 信息填写（Kind=ADM, SKind=DBZ, 选择 SKind2 + 名称/简称/命名者/wiki + 额外可选字段 + extensions）
...
export default function SettlementBoundaryDeterminedWorkflow(props: WorkflowComponentProps) {
  const { bridge } = props;
  ...
  if (step === 'creator') {
    return (
      <div className="p-3 rounded border border-gray-300 bg-white">
        <TopNav title="聚落范围-确定范围：填写者信息" showNext nextDisabled={!canGoNextFromCreator} onNext={() => setStep('info')} />
    ...
    return (
      <div className="p-3 rounded border border-gray-300 bg-white">
        ...
        <div className="space-y-3">
          <label className="block space-y-1">
            <div className="text-xs opacity-80">类型（Kind=ADM, SKind=DBZ）</div>
            <select
              className="w-full border p-1 rounded text-sm"
              value={info.skind2}
          ...
          <LabeledInput label="名称" value={info.name} placeholder="例如：某某聚落" onChange={(v) => setInfo((prev) => ({ ...prev, name: v }))} />
  ...
  return (
    <div className="p-3 rounded border border-gray-300 bg-white">
      <div className="flex items-center justify-between gap-2 mb-3">
        <div className="text-sm font-semibold">聚落范围-确定范围：绘制（地物面 / ISG）</div>
        <div className="flex items-center gap-2">
          <AppButton
            type="button"
