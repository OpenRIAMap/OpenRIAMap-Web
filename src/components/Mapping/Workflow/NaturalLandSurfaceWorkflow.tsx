 ...
 * 页面结构：
 * 1) 填写者信息（CreateBy）
 * 2) 信息填写（类型下拉 + 名称/简称/命名者/wiki + 所属大陆/所属聚落 + extensions）
...
type InfoForm = {
  ...
  brief?: string; // 简介
  landLevel1?: string; // tags.Land
  admLevel1?: string; // tags.Adm
};
...
export default function NaturalLandSurfaceWorkflow(props: WorkflowComponentProps) {
  const { bridge } = props;
  ...
  if (step === 'creator') {
    return (
      <div className="p-3 rounded border border-gray-300 bg-white">
        <TopNav title="自然要素-陆面要素：填写者信息" showNext nextDisabled={!canGoNextFromCreator} onNext={() => setStep('info')} />
  ...
  return (
    <div className="p-3 rounded border border-gray-300 bg-white">
      <div className="flex items-center justify-between gap-2 mb-3">
        <div className="text-sm font-semibold">自然要素-陆面要素：绘制（地物面 / ISG）</div>
        <div className="flex items-center gap-2">
          <AppButton
            type="button"
