// File: src/components/Mapping/Workflow/NaturalWaterwayWorkflow.tsx
...
export default function NaturalWaterwayWorkflow(props: WorkflowComponentProps) {
  const { bridge } = props;
  ...
  const commit = () => {
    ...
    try {
      ...
      const plineId = `${worldPrefix}${kind}${skind}${info.skind2}_${abbrNormalized}`;
  ...
  if (step === 'creator') {
    return (
      <div className="p-3 rounded border border-gray-300 bg-white">
        <TopNav title="自然要素-河道：填写者信息" showNext nextDisabled={!canGoNextFromCreator} onNext={() => setStep('info')} />
    ...
    return (
      <div className="p-3 rounded border border-gray-300 bg-white">
        ...
        <div className="space-y-3">
          ...
          <LabeledInput label={wfRegistry.getWorkflowFieldLabel(WF_KEY, 'Name')} value={info.name} placeholder="例如：北川" onChange={(v) => setInfo((prev) => ({ ...prev, name: v }))} />
  ...
  return (
    <div className="p-3 rounded border border-gray-300 bg-white">
      <div className="flex items-center justify-between gap-2 mb-3">
        <div className="text-sm font-semibold">自然要素-河道：绘制（地物线 / ISL）</div>
        <div className="flex items-center gap-2">
          <AppButton
            type="button"
      ...
      <div className="space-y-2">
        <div className="text-xs text-gray-600">绘制说明：当前已启用线绘制模式。请在地图上点击添加节点形成折线；完成后点击右上角“完成”。</div>
