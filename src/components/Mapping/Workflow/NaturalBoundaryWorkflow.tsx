...
export default function NaturalBoundaryWorkflow(props: WorkflowComponentProps) {
  const { bridge } = props;
  ...
  if (step === 'creator') {
    return (
      <div className="p-3 rounded border border-gray-300 bg-white">
        <TopNav title="自然要素-地理边界：填写者信息" showNext nextDisabled={!canGoNextFromCreator} onNext={() => setStep('info')} />
  ...
  return (
    <div className="p-3 rounded border border-gray-300 bg-white">
      <div className="flex items-center justify-between gap-2 mb-3">
        <div className="text-sm font-semibold">自然要素-地理边界：绘制（地物线 / ISL）</div>
        <div className="flex items-center gap-2">
          <AppButton
            type="button"
