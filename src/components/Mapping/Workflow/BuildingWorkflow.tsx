 ...
 * 1) 填写者信息（CreateBy）
 * 2) 信息填写（Class=BUD 下全部面要素：选择 Kind/SKind + 名称/简称/命名者/wiki + 额外可选字段 + extensions）
 * 3) BUD（建筑）绘制：完成后写入固定图层，并退出回到快捷测绘主页面
 */
...
export default function BuildingWorkflow(props: WorkflowComponentProps) {
  const { bridge } = props;
  ...
  if (step === 'creator') {
    return (
      <div className="p-3 rounded border border-gray-300 bg-white">
        <TopNav title="建筑：填写者信息" showNext nextDisabled={!canGoNextFromCreator} onNext={() => setStep('info')} />
  ...
  return (
    <div className="p-3 rounded border border-gray-300 bg-white">
      <TopNav title="建筑：绘制（面）" showPrev prevDisabled={false} onPrev={() => setStep('info')} />

      <div className="text-xs text-gray-700 mb-2">
        请在地图上绘制面要素（至少 3 个点）。完成后点击“保存”。
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <AppButton
          type="button"
          className="ml-auto px-3 py-1.5 rounded text-sm border hover:bg-gray-50"
          onClick={() => bridgeRef.current.exitWorkflowToSelector()}
        >
          返回
        </AppButton>
      </div>
