// File: src/components/Mapping/Workflow/NaturalLandWorkflow.tsx
...
const WF_KEY = 'ngf_land';

/**
 * NaturalLandWorkflow（工作流：自然要素-陆地）
 *
 * 页面结构：
 * 1) 填写者信息（CreateBy）
...
export default function NaturalLandWorkflow(props: WorkflowComponentProps) {
  const { bridge } = props;
  ...
  if (step === 'creator') {
    return (
      <div className="p-3 rounded border border-gray-300 bg-white">
        <TopNav title="自然要素-陆地：填写者信息" showNext nextDisabled={!canGoNextFromCreator} onNext={() => setStep('info')} />
    ...
    return (
      <div className="p-3 rounded border border-gray-300 bg-white">
        ...
        <div className="space-y-3">
          ...
          <LabeledInput label={wfRegistry.getWorkflowFieldLabel(WF_KEY, 'Name')} value={info.name} placeholder="例如：主岛" onChange={(v) => setInfo((prev) => ({ ...prev, name: v }))} />
  ...
  return (
    <div className="p-3 rounded border border-gray-300 bg-white">
      <div className="flex items-center justify-between gap-2 mb-3">
        <div className="text-sm font-semibold">自然要素-陆地：绘制（地物面 / ISG）</div>
        <div className="flex items-center gap-2">
          <AppButton
            type="button"
